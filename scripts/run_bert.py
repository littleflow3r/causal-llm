...
from utils_bert import RelationLoader, SemEvalDataLoader
from model_bert import R_BERT
#from model_bert_orig import R_BERT
...
class Runner(object):
    def __init__(self, id2rel, loader, user_config):
        ...
        self.eval_tool = Eval(user_config, self.class_num)
    ...
    def train(self):
        ...
        # num_training_steps = int(num_training_steps* 2 )
        num_warmup_steps = int(num_training_steps *
                               self.user_config.warmup_proportion)

        print ('t.steps, w.steps:', num_training_steps, num_warmup_steps)
        ...
        for epoch in range(1, 1+self.user_config.epoch):
            ...
            f1, dev_loss, predict_label, true_label = self.eval_tool.evaluate(self.model, dev_loader)
    ...
    def test(self):
        ...
        f1, test_loss, predict_label, true_label = self.eval_tool.evaluate(self.model, test_loader)
...
def print_result(floc, predict_label, true_label, id2rel, start_idx=0):
    assert len(predict_label) == len(true_label), 'assert error len true!= len predict label'
    cv = floc.split('/')[-2]
    # ./resource/data_gene/1/GENE-PubBERT-b8e10g1
    des_file = floc+'/'+str(cv)+'_result_bert.txt'
    print (des_file)
    with open(des_file, 'w', encoding='utf-8') as fw:
        fw.write('{}\t{}\t{}\n'.format('index', 'true', 'prediction'))
        for i in range(0, predict_label.shape[0]):
            fw.write('{}\t{}\t{}\n'.format(
                start_idx+i, true_label[i], predict_label[i]))
...
if __name__ == '__main__':
    ...
    if user_config.mode == 0:  # train mode
        runner.train()
        predict_label, true_label = runner.test()
    elif user_config.mode == 1:
        predict_label, true_label = runner.test()
    else:
    ...
    # # start_idx = 0
    # # for i in range(0, predict_label.shape[0]):
    # #     print (start_idx+i, predict_label[i])
    print_result(user_config.model_dir, predict_label, true_label, id2rel)
